...
export default class jagc extends Route {
    constructor(server: Server) {
        super(server, {authOnly: false, path: '/jagc', deprecated: false, maintenance: false });
    }
    ...
    public bind() {
        this.router.get('/', async (req, res) => {
            try {
                ...
                const homePath = path.join(process.cwd() + '/public/jagc/index.html');
                this.router.get('/', function (req, res) {
                    res.sendFile(homePath);
                })


                this.router.get('/active', forceAuth, function (req, res) {
                    res.send('hi');
                });
